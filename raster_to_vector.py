...
def createvector(geojson_path,tif_path,s3file_name):
    ...
    # writeJson(aws_access_key,aws_secret_key,aws_bucket,data,s3file_name)
        with open('/home/azureuser/ndvirectified/district/'+str(s3file_name), 'w') as f:
            json.dump(data, f)
        f.close()

arr = os.listdir('/home/azureuser/ndvirectified/latest')
for i in arr:
    without_ext=os.path.splitext(i)[0]
    print(without_ext)
    createvector('/home/azureuser/with_id/TS_mandal_boundary.json','/home/azureuser/ndvirectified/latest/'+str(i),str(without_ext)+'.geojson')